...
def main():
    ...
    parser.add_argument('--debug', default=False,
        help='if debug, then train_debug_32.txt is used for training (WordCorpus)')
    parser.add_argument('--colab_gpu', default=False,
        help='for debugging: set to True if you are running on colab gpu and using pretrained embeddings.')
    parser.add_argument('--embed_source', choices=["en_vectors_web_lg", "en_core_web_lg"],
        default="en_vectors_web_lg", help="choose source of vectors. if CPU, choose default")
    parser.add_argument('--debug_vectors_option', default="one", choices= ["one", "two", "three", "four"],
        help="see colab notebook for the options (1-4). 1: original approach, should be used with en_vectors_web_lg.")
    parser.add_argument('--trainable_embeds', default=False,
        help="if True, then the trainable embeddings are used, otherwise a static embedding layer")
    ...
    if args.trainable_embeds:
        print("running with trainable pretrained embeddings")
    else:
        print("running with pretrained embeddings (not trainable)")
    ...
    for seed in args.seed:
        ...
        if args.debug:
            corpus = data.WordCorpus(args.data, freq_cutoff=1, verbose=True, train="train_debug_32.txt", args=args)
        else:
            freq_cutoff = args.unk_threshold * 2
            corpus = data.WordCorpus(args.data, freq_cutoff=freq_cutoff, verbose=True, args=args)
        if args.test_corpus == 'full':
            ...
            test_corpus = data.WordCorpus(args.data, train='train_uncorrelated.txt', valid='valid_uncorrelated.txt', test='test_uncorrelated.txt',
                freq_cutoff=freq_cutoff, verbose=True, word_dict=corpus.word_dict, args=args)
        ...
        if args.test_only:
            ...
            model = SelectModel(corpus.word_dict, corpus.output_length, args, device, corpus.pretrained_embeds)
