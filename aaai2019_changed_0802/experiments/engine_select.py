...
class SelectEngine(object):
    ...
    def forward(model, batch, requires_grad=False):
        ...
        with torch.set_grad_enabled(requires_grad):
            ...
            # create variables
            ctx = Variable(ctx) # shape (16,28)
            inpt = Variable(inpt) # shape (140,16)
            tgt = Variable(tgt)
            sel_tgt = Variable(sel_tgt) # shape (16)
            ...
            #pdb.set_trace()
            ctx_h = model.forward_context(ctx) # shape (1,16,128)
            lang_h = model.zero_hid(ctx_h.size(0), model.args.nhid_lang) # shape (1,1,128)
            sel_out = model.forward_selection(inpt, lang_h, ctx_h) # error: input.size(-1) must be equal to input_size. Expected 128, got 300
